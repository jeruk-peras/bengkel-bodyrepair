<script>
    ...
    var table = $('#datatable').DataTable({
        ...
        ajax: {
            url: '/datatable-server-side/karyawan', // URL file untuk proses select datanya
            type: 'POST',
            data: {
        ...
        columnDefs: [{
            ...
            render: function(data, type, row, meta) {
                var btn =
                    '<a href="/karyawan/' + data + '/edit" class="me-2 btn btn-sm btn-primary btn-edit" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Edit Data"><i class="bx bx-pencil me-0"></i></a>' +
                    '<a href="/karyawan/' + data + '/delete" class="me-2 btn btn-sm btn-danger btn-delete" data-id-produk="' + data + '" data-bs-toggle="tooltip" data-bs-placement="top" data-bs-original-title="Hapus Data"><i class="bx bx-trash me-0"></i></a>'
                return btn;
            }
        }, ],
        pageLength: 25,
    ...
    table.on('click', 'tbody tr td a.btn-edit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                dataCabang(response.data.cabang_id);
            },
            error: function(xhr, status, error) {
                var response = JSON.parse(xhr.responseText);
                alertMesage(response.status, response.message);
